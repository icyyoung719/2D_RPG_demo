...
void Map::LoadTiledMap(const std::string& jsonPath) {
	// 清空现有地图数据
	tileMap.clear();

	// 加载Tiled格式地图
	std::cout << "Loading Tiled Map: " << jsonPath << std::endl;
	if (mapLoader.loadFromFile(jsonPath, *this)) {
	std::cout << "Tiled Map loaded successfully" << std::endl;
	} else {
	std::cerr << "Failed to load Tiled Map: " << jsonPath << std::endl;
	}
}
